...
if __name__ == '__main__':
    ...
    #recordar los rangos de amariilo para deterctar pato :
        #    lower_yellow = np.array([15,155,155])
        #   upper_yellow = np.array([45,255,255])


        
        #IMPORTANTE: NUEVOS RANGOS PARA DETECTAR AZUL DE DUCKIEBOT, NO SE MODIFICA EL NOMBRE "YELLOW" PARA EVITAR FUTUROS ERRORES JAJA
    lower_yellow = np.array([90,190,120])
    ...
    def seguir_centro(error,area):
        k = 0.01
        if  area < 13000:
            return np.array([0.2, k*error])
        elif 20000 > area> 13000: 
            return np.array([0.1, k*error])
        elif area > 20000 :
            return np.array([0.0001, k*error])
        else:
            return np.array([0.2, k*error])
    ...
    while True:
       ...
       #     env.reset()
        ...
        if seguir:
            action = seguir_centro(error,area)
